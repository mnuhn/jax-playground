...
def check_column(data,
                 col_name,
                 min_non_nan_percent=99,
                 min_non_zero_percent=90,
                 min_min=None,
                 max_max=None):
  ...
  if not ok:
    print(
        f"  {col_name}: non-nan: {non_nan_percent:.1f}%, non-zero: {non_zero_percent:.1f}%, min: {col_min}, max: {col_max}"
    )
...
def inspect(filtered_data):
  ...
  ok &= check_column(filtered_data,
                     "wind_speed",
                     min_non_zero_percent=20.0,
                     min_min=0.0,
                     max_max=35.0)
  ok &= check_column(filtered_data,
                     "gust_speed",
...
start = datetime.date(int(data[min_idx, 0]), int(data[min_idx, 1]),
                      int(data[min_idx, 2]))
end = datetime.date(int(data[max_idx, 0]), int(data[max_idx, 1]),
                    int(data[max_idx, 2]))
...
while d <= end:
  ...
  if skip(d.year):
    print(
        f"skipping {d} due to selected year range [{args.min_year}, {args.max_year}]"
    )
    continue
