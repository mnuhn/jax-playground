...
class Visualizer():
  ...
  def draw_dict(self, debug, num, step):
    ...
    for k in debug.keys():
      ...
      self.id.text((x1 + self.box_width, y0),
                   k,
                   font=font,
      ...
      if k.startswith("conv_") and (k.endswith("conv") or k.endswith("relu")
                                   ):  # TODO or k == "input_conv":
        x0 = x1 + self.box_width
      else:
        x0 = 0
        y0 = y1 + self.box_width

    self.id.text((0, y_max),
                 f"step: {step}",
                 font=font,
                 fill=(255, 255, 255, 128))
  ...
  def box(self, arr, x0, y0):
    for i in range(arr.shape[0]):
      for j in range(arr.shape[1]):
        v = int(arr[i, j])
        self.id.rectangle([(x0 + i * self.box_width, y0 + j * self.box_width),
                           (x0 + (i + 1) * self.box_width, y0 +
                            (j + 1) * self.box_width)],
                          fill=(v, v, v))
    self.id.rectangle([
        (x0, y0),
        (x0 + arr.shape[0] * self.box_width, y0 + arr.shape[1] * self.box_width)
    ],
                      outline='red',
                      width=1)

    return (x0 + arr.shape[0] * self.box_width,
            y0 + arr.shape[1] * self.box_width)
